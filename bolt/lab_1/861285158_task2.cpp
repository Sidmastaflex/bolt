...
facility_ms *f;                 /*the service center*/ 

void car() 
{ 
    create("car");        /*make this a process*/ 
    f->use(uniform(10,60));   /*obtain needed amount of service*/ 
} 

extern "C" void sim() {
	create("sim");            /*make this a process*/ 
	f = new facility_ms("Gas Station", 10);      /*create the service center - f*/ 
	
	while(simtime() < 10000) {  /*loop until end of simulation*/ 
		double liters = uniform(10,60);

		if (!(f->status() != 0 && balkingStream->nextrand() > (40 + liters) / (25 * (3 + f->qlength())))){
			f->reserve();
			hold(uniform(12,20));
			f->release();
		}
